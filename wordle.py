...
class Wordle(commands.Cog):
    ...
    def __init__(self, client):
        ...
        try:
            ...
            version_data = requests.get(version_url).json(encoding='utf-8')
            version = version_data['v']

            # Fetch champion data
            champion_data_url = f'http://ddragon.leagueoflegends.com/cdn/{version}/data/en_US/champion.json'
            response = requests.get(champion_data_url).json(encoding='utf-8')
    ...
    def image_for_hidden_word(self):
        padding = 5
        image = Image.new(mode='RGBA', size=(Wordle.LETTER_SIZE * self.size, Wordle.LETTER_SIZE), color=(0, 0, 0, 0))
        ...
        for i, c in enumerate(self.last_guess):
            ...
            draw.rectangle((left - padding, top - padding, right + padding, bottom + padding), fill=bg)
